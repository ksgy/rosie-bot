...
RosieBot.prototype._reply = function (originalMessage) {
	...
	if (command.command === 'ORDER') {
        var basketItem = rosieHelper.getMenuItemByName(this.menu, command.item, command.amount);
        if (!basketItem) {
            console.log('Not valid basket item', basketItem);
            self.postMessage(originalMessage.channel, 'Oh dear! :scream: I\'m afraid, I couldn\'t find ' + command.item + ', my darling... :thinking_face: but here\'s a ' + rosieHelper.getCompensation() + '  for you!' + rosieHelper.getHelpText(true), { as_user: true });
            return;
        }
