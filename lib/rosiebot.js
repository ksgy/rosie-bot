...
RosieBot.prototype._reply = function (originalMessage) {
	...
	var basketItemUUID = rosieHelper.getMenuItemByName(this.menu, itemName);
	if (!basketItemUUID) {
		console.log('Not valid basketUUID', basketItemUUID)
	...
	rosieHelper.createBasket(basketItems)
		...
		.catch(function(error){
			self.postMessage(originalMessage.channel, 'Oh dear! :scream: Chef told me ' + error.response.data + ', my darling... but here\'s a :cake: for you!', { as_user: true });
		});
};
