...
var rosieHelper = {
	getMenu: function(){
		return axios.get('https://api.foodit.com/restaurant/sushisurpriseshoreditch');
	},
	...
	createBasket: function(items){
		...
		return axios.post('https://api.foodit.com/basket', {"restaurantId":5888831364530176,"restaurantCode":"sushisurpriseshoreditch","items": items});
	},
	createFulfillment: function(response){
		console.log('# 2 - createFulfillment');
		var basketId = response.data.uuid;
		return axios.put('https://api.foodit.com/basket/'+basketId+'/fulfillment', {delivery: "false"});
	},
	setPayment: function(response){
		console.log('# 3 - setPayment');
		var basketId = response.data.uuid;
		return axios.put('https://api.foodit.com/basket/'+basketId+'/payment', {paymentMethod: "CASH"});
	},
	doOrder: function(response){
		console.log('# 4 - doOrder');
		var basketId = response.data.uuid;
		return axios.post('https://api.foodit.com/basket/'+basketId+'/order', {"redirectUrl":"http://localhost:3000/confirmation/{orderId}?status=submitted","noPaymentUrl":"http://localhost:3000/checkout/"+basketId,"hostedFieldsPayment":false,"paymentMethod":"CASH","comments":"Test order","consumer":{"name":"test","email":"test@foodit.com","phone":"07000000000"}});
	},
